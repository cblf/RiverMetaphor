...
var setupInstance = function (s1) {
   

    s1.setup = function () {
        s1.createCanvas(1728, 1080);
        s1.background(WHITE);
        canvasWidth = 1728;// s1.canvasWidth;
        canvasHeight = 1080/*s1.canvasHeight*/ - headerHeight - footerHeight;
        originX = 0;
    ...
    var drawConceptsStrengths = function() {
        ...
        for (var c = 0; c < NB_CONCEPTS; c++) {
            ...
            s1.stroke(red(currentColor) / 2, green(currentColor) / 2, blue(currentColor) / 2, 127);
            //noFill();
            s1.fill(currentColor);
            s1.curveVertex(0, originY);
            ...
            for (var i = 0; i < hsKeys.length; i++) {
                ...
                s1.curveVertex(posX, posY);
                //print(posX + " " + posY + " " + posScore);

                sum[c % 2][i] += hs.conceptScores[c];
            }
            s1.curveVertex(canvasWidth, originY);
            s1.curveVertex(canvasWidth, originY);

            s1.curveVertex(canvasWidth / 2, originY);
            s1.curveVertex(0, originY);
        ...
        for (var c = 0; c < NB_CONCEPTS; c++) {
            ...
            for (var i = 0; i < hsKeys.length; i++) {
                ...
                if (sumOfScores < sum[c % 2][i]) {
                    ...
                    s1.text("S" + c, posX, posY + (15 * (c % 2)));
                }

                // Etiquettage
                s1.strokeWeight(10); 
                if (i == 0) {
                    s1.text("Début " + c, posX - 120, posY + (15 * (c % 2)));
                }
                if (hs.conceptScores[c] == _maxScorePerConcept[c]) {
                    s1.stroke(0, 255, 0);
                    s1.point(posX, posY);
                    s1.strokeWeight(1); s1.stroke(0, 0, 0); s1.fill(BLACK);
                    s1.text("Max Sujet " + c, posX, posY + (15 * (c % 2)));
                    ...
                    s1.point(posX, posY);
                    s1.strokeWeight(1); s1.stroke(0, 0, 0); s1.fill(BLACK);
                    s1.text("Min Sujet " + c, posX, posY + (15 * (c % 2)));
                    //print("trouvé min sujet " + c + " en " + i + " : " + hs.conceptScores[c]);
                }
                else
                { s1.stroke(0, 0, 255); s1.point(posX, posY); }
    ...
    var drawAxis = function () {
        ...
        s1.strokeWeight(1);
        s1.stroke(BLACK);
        s1.line(originX, originY, originX + canvasWidth, originY);
        s1.line(originX, headerHeight, originX + canvasWidth, headerHeight);
        s1.line(originX, height - footerHeight, originX + canvasWidth, height - footerHeight);
        ...
        for (var i = 0; i <= n2; i++) {
            ...
            s1.strokeWeight(3);
            s1.stroke(color(0, 0, 0, 127));
            s1.line((float)(i * deltaX), yy, (float)(i * deltaX), yy2);
            s1.strokeWeight(1);
            s1.stroke(color(255, 255, 255, 127));
            s1.line((float)(i * deltaX), yy, (float)(i * deltaX), yy2);
        }
